
## 🇧🇷 Português

Finanças

Finanças é um sistema projetado para ajudar na organização e no gerenciamento das suas contas, contribuindo para uma boa gestão financeira.

Devido à dificuldade que muitas pessoas enfrentam para administrar o dinheiro no dia a dia, o objetivo do sistema é ser uma ferramenta de apoio à organização financeira pessoal, permitindo registrar e acompanhar todas as despesas.



## Tecnologias utilizadas

* Java
* Spring Boot
* MySQL
* JavaScript
* HTML
* CSS
* Node.js
* Maven
* Claude
* Gemini
* Figma


## Funcionalidades 

* Cadastro de despesas
* Gráfico 
* opções de gráficos: barras, linhas, rosca e radar
* Registro de saldo: entrada e saída de dinheiro
* Planilha
* Espaço para definir metas de gastos e reserva de emergência.
* Calculadora

## ARQUIVOS

Financas.email.api é servidor do sistema finanças, sem ele o sistema não funciona.
Financas-app é arquivo que contém o sistema finanças.


## MANUAL DE INSTALAÇÃO

[Clique aqui para obter o passo a passo de instalação](./Financas-versao-alfa/PASSO%20A%20PASSO%20DE%20INSTALAÇÃO.txt)

## SOBRE O DESENVOLVIMENTO

A interface e servidor do sistema foram desenvolvidos com base neste projeto original do netBeans: [Finanças.zip](https://github.com/user-attachments/files/30905677/Financas.zip) 
Para construção de uma interface moderna foram utilizados Claude, Gemini e Figma para auxílio no desenvolvimento do front-end.






🇺🇸 English

# Finances

Finances is a system designed to help organize and manage your accounts, contributing to sound financial management.

Due to the difficulty many people face in managing their daily money, the system's objective is to serve as a support tool for personal financial organization, allowing users to record and track all expenses.

---

##  Technologies Used

* Java
* Spring Boot
* MySQL
* JavaScript
* HTML
* CSS
* Node.js
* Maven
* Claude
* Gemini
* Figma

---

## Features

* Expense registration
* Charts: bar, line, donut, and radar
* Balance tracking: income and expense entries
* Spreadsheet
* Dedicated space to set goals
* Calculator

---

## Files

* **Financas.email.api:** The server for the Finances system (required for the system to run).
* **Financas-app:** The repository file/folder containing the main Finances application.

---

## 📖 Installation Manual

📄 [Click here to get the step-by-step installation guide](https://github.com/Souza1932/Finan-as/blob/main/Financas-versao-alfa/INSTALLATION_STEP_BY_STEP.txt)

---

## About the Development

The system's interface and server were developed based on this original NetBeans project: [Finanças.zip](https://github.com/user-attachments/files/30905677/Financas.zip)
To build a modern interface were used Claude,Gemini and Figma as assistance tools during front-end development.





## TELA DE LOGIN

<img width="1918" height="1055" alt="Captura de ecrã de 2026-08-10 13-43-10" src="https://github.com/user-attachments/assets/fe9deb8a-88ba-4cee-a586-f1bf1cea31cc" />



## TELA INICIAL 


<img width="1857" height="991" alt="Captura de ecrã de 2026-08-10 13-46-45" src="https://github.com/user-attachments/assets/18e189dd-8348-4296-855f-19c5ae7d4de5" />


<img width="1857" height="989" alt="Captura de ecrã de 2026-08-10 13-44-03" src="https://github.com/user-attachments/assets/1f2b3929-926f-492a-8a81-b3018086cdfc" />


<img width="1857" height="991" alt="Captura de ecrã de 2026-08-10 13-47-21" src="https://github.com/user-attachments/assets/ea38cb0e-7886-4ee2-8fd5-f8b61f546964" />


<img width="1857" height="991" alt="Captura de ecrã de 2026-08-10 13-47-14" src="https://github.com/user-attachments/assets/a68566bb-a908-4e5f-96be-8af170d37f8c" />


## TELA DE CADASTRO DE DESPESAS


<img width="1857" height="1046" alt="Captura de ecrã de 2026-08-10 13-44-12" src="https://github.com/user-attachments/assets/525cb9a5-ce95-407c-9b64-7104c7b66a8d" />


## REGISTRAR SALDO

<img width="1857" height="1046" alt="Captura de ecrã de 2026-08-10 13-44-20" src="https://github.com/user-attachments/assets/3c4a93f2-bd9d-4e13-8df8-d223effa01d5" />



## REGISTRO DE DESPESAS


<img width="1857" height="1046" alt="Captura de ecrã de 2026-08-10 13-44-28" src="https://github.com/user-attachments/assets/32b36fdb-2070-49b8-a7e4-e88d7734cfee" />


<img width="1857" height="1046" alt="Captura de ecrã de 2026-08-10 13-44-34" src="https://github.com/user-attachments/assets/048048c7-7050-449a-b9ad-fbc66b1b186d" />


## DEFINIR METAS


<img width="1857" height="1046" alt="Captura de ecrã de 2026-08-10 13-44-42" src="https://github.com/user-attachments/assets/5f144bc0-0b84-4ada-aa5d-36f408bea96e" />


## CALCULADORA


<img width="1857" height="1046" alt="Captura de ecrã de 2026-08-10 13-44-50" src="https://github.com/user-attachments/assets/abe7e061-a0c0-4094-a8b6-ddcd8a583480" />



## TELA DE PERFIL


<img width="1857" height="1046" alt="Captura de ecrã de 2026-08-10 13-45-19" src="https://github.com/user-attachments/assets/17032f8f-55b9-4e9c-86c3-b9126746c607" />





